...
OpenClaw 是一个**自托管的网关（Self-hosted Gateway）**，它将你最喜欢的聊天应用（WhatsApp、Telegram、Discord、iMessage 等）与 AI 编程代理（如 Pi）连接起来。

**核心特点**：
- 在本地机器或服务器上运行单一 Gateway 进程
- 成为聊天应用和始终在线的 AI 助手之间的桥梁
- 为开发者和高级用户打造，让他们可以从任何地方获得个人 AI 助手
...
**关键区别**：OpenClaw 不是聊天机器人，而是能真正做事的 Agent 平台！
...
**核心思想**：这不是简单的聊天，而是有状态的、能调用工具执行任务的智能循环！
...
#### 1️⃣ 运行时工具 (Runtime Tools)
- `exec` - 执行 shell 命令
- `process` - 管理后台进程
- `gateway` - 重启/更新 Gateway

#### 2️⃣ 文件系统工具 (Filesystem Tools)
- `read` - 读取文件（支持文本 + 图片）
- `write` - 创建/覆盖文件
- `edit` - 编辑文件（精确替换）
- `apply_patch` - 应用 patch

#### 3️⃣ 会话工具 (Session Tools)
- `sessions_list` - 列出会话
