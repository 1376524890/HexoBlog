...
> **准备时间**: 2026 年 3 月 18 日 07:05 AM (Asia/Shanghai)  
> **学习时长**: ~3 小时 + 实时补充  
> **用途**: 明早 7 点的汇报准备  
> **整理者**: 御坂美琴一号 ⚡
...
> **OpenClaw 是一个自托管的 AI Agent 运行时网关平台**，核心是把 AI 连接到真实世界的桥梁。  
> 它不是聊天机器人，而是能真正执行任务的 Agent 平台。
...
| **Channel** | 协议适配器 | Telegram、Discord、飞书、微信等 15+ 个 |
...
### 官方介绍
> OpenClaw 是一个自托管网关，连接你最爱的聊天应用（WhatsApp、Telegram、Discord、iMessage 等）到 AI 编码代理（如 Pi）。你在自己的机器（或服务器）上运行单个 Gateway 进程，它成为聊天应用和始终可用的 AI 助手之间的桥梁。

**目标用户**: 开发者和高阶用户，希望随时随地拥有个人 AI 助手，同时完全掌控数据。
