...
class RJinaAI:
    ...
    async def fetch(self, url: str) -> RJinaAIResult:
        """使用 r.jina.ai 抓取 URL"""
        start_time = time.time()
        
        try:
            jina_url = f"{self.BASE_URL}/{url}"
            scraper_logger.info(f"抓取 r.jina.ai: {jina_url}")
            
            async with self.session.get(jina_url, allow_redirects=True) as response:
                if response.status == 200:
                    content = await response.text()
                    return RJinaAIResult(
    ...
    def fetch_sync(self, url: str) -> RJinaAIResult:
        return asyncio.run(self.fetch(url))


r_jina_ai = RJinaAI()

def fetch_r_jina_ai(url: str) -> RJinaAIResult:
    return r_jina_ai.fetch_sync(url)

async def async_fetch_r_jina_ai(url: str) -> RJinaAIResult:
    async with RJinaAI() as scraper:
