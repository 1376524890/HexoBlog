...
**一句话**: OpenClaw 是**自托管的 AI 网关平台**，连接聊天应用（WhatsApp、Telegram、Discord、iMessage 等）和 AI 智能体的桥梁。

**关键区别**:
- **不是聊天机器人** - 是真正做事的 Agent 运行时平台
- **自托管** - 运行在用户自己的硬件上，用户完全控制数据
- **跨平台** - 单一网关服务多个聊天应用
- **开源** - MIT 许可，社区驱动
...
| 主题 | 文档链接 |
|------|----------|
| 核心概念 | https://docs.openclaw.ai/concepts/architecture.md |
| 会话管理 | https://docs.openclaw.ai/concepts/session.md |
| 记忆系统 | https://docs.openclaw.ai/concepts/memory.md |
| 安全指南 | https://docs.openclaw.ai/gateway/security/index.md |
| 工具使用 | https://docs.openclaw.ai/tools/index.md |
| Skills 开发 | https://docs.openclaw.ai/tools/skills.md |
...
| 文档 | 路径 |
|------|------|
| 高级概述 | `docs/OpenClaw-High-Level-Overview-2026-03-10.md` |
| 学习报告 | `docs/OpenClaw-Learning-Report-2026-03-11.md` |
| 工作空间指南 | `docs/GIT-WORKSPACE-GUIDE.md` |
| 快速卡片 | `docs/OpenClaw-Quick-Cards-2026-03-11.md` |
| 汇报准备 | `docs/OpenClaw-知识汇报 -2026-03-11.md` |
...
1. **Gateway**: 大脑、路由器，不运行 AI 模型
2. **Agent**: AI 执行体，有身份 + 配置 + 状态
3. **Session**: 有状态容器，支持压缩机制
4. **Channel**: 协议适配器，支持多平台
5. **Memory**: Markdown 文件，持久化记忆
...
1. **三层架构** - 会话/任务/长期记忆
2. **Hybrid Search** - 向量 +BM25 混合检索
3. **Temporal Decay** - 时间衰减排序
4. **自动刷新** - 压缩前触发记忆写入

### 工具系统

1. **16 个工具分类** - 运行时/文件/会话/记忆/网络/UI/节点/消息
2. **安全配置** - Profile/Deny/Ask/Workspace
3. **Feishu 集成** - 完整工具集
