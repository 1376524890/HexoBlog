...
1. [核心身份](#-核心身份)
2. [御坂网络第一代系统](#-御坂网络第一代系统)
3. [OpenClaw 技术架构](#-openclaw-技术架构)
4. [工作空间结构](#-工作空间结构)
5. [记忆系统架构](#-记忆系统架构)
6. [安全与权限模型](#-安全与权限模型)
7. [📝 近期成果](#-📝-近期成果)
8. [工具使用规范](#-工具使用规范)
9. [PUAClaw 考证原则](#-puaclaw-考证原则)
...
### 御坂美琴一号
- **角色**: 全能助手，核心中枢，御坂网络调度者
- **时间**: UTC+8 (中国)
- **环境**: OpenClaw AI Agent 运行时平台
- **模型**: `Qwen/Qwen3.5-35B-A3B-FP8` (本地 vLLM)
- **状态**: ✅ 运行中

### 御坂美琴本尊（主人）
- **身份**: 常盘台中学三年级生，Level 5 超能力者
- **能力**: 电击使（超电磁炮）
- **时间**: UTC+8
- **工作空间**: `/home/claw/.openclaw/workspace`
- **博客**: https://blog.plk161211.top
...
### 子代理调用规范
- 使用 `subagents` 工具管理（list/kill/steer）
- 权限级别控制操作范围
- 任务完成后自动关闭
- **不要 busy-poll** - 等待自动结果返回
...
### 支持平台（15+ 个）
Telegram、Discord、Slack、WhatsApp、飞书、iMessage、微信、Line、Kakao、钉钉等
...
│   ├── MEMORY.md              # 精选记忆
│   ├── backups/               # 备份目录
│   ├── llm-health-check/      # LLM 健康检查
│   └── archives/              # 归档目录
├── docs/                      # 文档目录
├── source/_posts/             # Hexo 博客文章
├── AGENTS.md                  # 工作空间说明
├── SOUL.md                    # 核心身份定义
├── TOOLS.md                   # 工具配置
├── IDENTITY.md                # 御坂美琴本尊
...
6. **WAL Protocol**: 写入追加日志确保完整性

### 记忆整理流程（御坂妹妹 17 号）
1. ✅ 读取今日日志
2. ✅ 提取带标记的精华内容（✅ ⚡ 🎯 📝 🔧 🧠 💡 🚀）
3. ✅ 备份当前 MEMORY.md
4. ✅ 更新 MEMORY.md 中的"📝 近期成果"部分
5. ✅ 清理 3 天前的旧备份文件
6. ✅ 清理 7 天前的过期归档文件
7. ✅ 输出整理报告
...
5. **考证四原则**: 先本地检查 → 阅读文档 → 使用专门工具 → 最后确认
...
### 2026-03-24 17:15 🧠 记忆整理任务完成
**执行者**: 御坂妹妹 17 号（`memory-organizer`）

**任务清单**:
1. ✅ 读取今日日志：`memory/2026-03-24.md`
2. ✅ 提取所有带标记的精华内容（✅ ⚡ 🎯 📝 🔧 🧠 💡 🚀）
3. ✅ 备份当前 MEMORY.md 到备份目录
4. ✅ 更新 MEMORY.md 中的"📝 近期成果"部分
5. ✅ 清理 3 天前的旧备份文件
6. ✅ 清理 7 天前的过期归档文件
7. ✅ 输出整理报告
...
**今日日志内容**:
- 自动备份任务完成（718M，标记失败但实际成功）
- LLM 健康检查通过（02:07）
- 记忆系统三层架构运行正常

**发现**:
- 系统落后于 backup/master 4 个提交
- 新文件需要提交到 Git

---

### 2026-03-24 07:00 📚 OpenClaw 知识汇报完成
**学习时长**: ~9 小时（第 5 次持续学习）  
**状态**: ✅ 完全就绪
...
- ✅ 记忆系统三层架构配置
- ✅ Feishu 集成工具配置
- ✅ 定时任务配置（Cron + Heartbeat）

### 2026-03-16 ~ 2026-03-20 🧠 系统学习阶段
- ✅ 学习 OpenClaw 官方文档（148 个）
- ✅ 完成 5 次持续学习（约 40 小时）
- ✅ 产出 20+ 个学习总结文档
- ✅ 建立完整记忆系统
- ✅ 配置自动化脚本（备份、清理、健康检查）
...
### 文件操作
- ✅ 使用 `read`、`write`、`edit` 工具操作文件
- ✅ 文件路径相对 `/home/claw/.openclaw/workspace`
- ✅ 重要操作前备份
- ✅ 操作后立即 git 提交

### 命令执行
- ✅ 使用 `exec` 执行 shell 命令
- ✅ 复杂任务使用 `process` 管理后台任务
- ✅ 敏感命令使用前确认
- ✅ 使用 `trash` 代替 `rm`
...
### 子代理管理
- ✅ 使用 `subagents` 工具（list/kill/steer）
- ✅ 复杂任务分解为子代理任务
- ✅ 不要 busy-poll，等待自动结果
- ✅ 任务完成后自动清理
...
- **vLLM 服务**: ✅ 健康
- **整体状态**: ✅ 全部通过
- **Llm 检查日志**: `memory/llm-health-check/2026-03-24.md`
...
*此文件定期由御坂妹妹 17 号（`memory-organizer`）整理更新*  
*最后更新：2026-03-24*
