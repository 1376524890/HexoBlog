...
**背景**：
- SP1 我们建立了 OpenClaw 的网关视角全景图
- 四大组件中，**Agent**是最核心的 AI 执行体
...
| 等级 | 编号 | 权限范围 | 适用场景 |
|------|------|----------|----------|
| Level 1 | 只读 | 仅读取文件 | 数据分析、日志查看 |
| Level 2 | 受限 | 指定目录读写 | 通用代理、文件管理 |
| Level 3 | 标准 | 工作目录读写 | 代码执行、内容创作 |
| Level 4 | 受限系统 | 系统配置（需批准） | 系统管理员 |
| Level 5 | 完全 | 完全访问权限 | 主 Agent |
...
**定义**：系统提示词（System Prompt）是 Agent 的"出厂设置"，定义了：
- **身份**：你是谁？
- **能力**：你能做什么？
- **规则**：你应该如何行为？
- **环境**：你现在在哪里？

**SP1 回顾**：SP1 中提到系统提示词是动态生成的，本章深入其内部机制。
