...
御坂大人发现：
- **本地模型**（Qwen3.5-35B）：需要本地资源，Token 消耗在自己账号
- **远程模型**（Kimi K2.5）：性能优秀，但有成本！需要仔细评估使用量
...
| 模型 | 用途 | 优势 | 说明 |
|------|------|------|------|
| **kimi-k2.5** | 主力模型 | 长上下文（262K）、推理能力强 | ✅ 需要付费，但性价比不错 |
| **glm-5** | 备用模型 | 作为 fallback，稳定性高 | ✅ 可以作为成本优化的选项 |
| **Qwen3.5-35B** | ❌ 废弃 | 本地模型，性能不如 Kimi | ❌ 需要消耗本地 GPU 资源 |
